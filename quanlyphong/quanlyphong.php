...
<div class="container">
    ...
    <form method="GET" action="quanlyphong.php">
        <div class="facility-buttons">
            <button type="submit" name="facility_id" value="1" class="facility-button">Cơ sở 1</button>
            <button type="submit" name="facility_id" value="2" class="facility-button">Cơ sở 2</button>
        </div>
    </form>
    ...
    <?php elseif ($result->num_rows > 0): ?>
        <div class="room-container">
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="room">
                    <h3>Phòng: <?php echo $row["room_name"]; ?></h3>
                    <p>Cơ sở: <?php echo $row["facility_name"]; ?></p>
                    <p>Giá: <?php echo number_format($row["price"], 0, ',', '.'); ?> VND</p>
                    <p>Trạng thái: <?php echo ucfirst($row["status"]); ?></p>
