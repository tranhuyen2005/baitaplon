                ...
                <div class="row mt-4">
                    ...
                    <?php while ($room = $result_rooms->fetch_assoc()): ?>
                        ...
                        <div class="col-md-4 mb-4">
                            <div class="room-card <?php echo $room_class; ?>">
                                <h5><?php echo $room['room_name']; ?></h5>

                                <!-- Hiển thị tình trạng phòng với màu sắc -->
                                <p class="status-text" style="color: <?php echo $room['status'] == 'empty' ? 'green' : 'blue'; ?>;">
                                    <?php echo $room['status'] == 'empty' ? 'Còn trống' : 'Đã cho thuê'; ?>
                                </p>

                                <!-- Hiển thị chủ sở hữu hoặc Không có -->
                                <p>Chủ sở hữu: <?php echo $room['status'] == 'empty' ? 'Không có' : $room['owner_name']; ?></p>

                                <!-- Hiển thị thông tin giá -->
                                <p style="color: <?php echo $room['status'] == 'empty' ? 'green' : 'black'; ?>;">
                                    Giá: <?php echo number_format($room['price'], 0, ',', '.'); ?> VNĐ
                                </p>

                                <!-- Hiển thị địa chỉ -->
                                <p>Địa chỉ: <?php echo $room['address']; ?></p>

                                <!-- Các nút thao tác -->
                                <div class="btn-group">
                                    <button class="btn btn-sm btn-success">Trả phòng</button>
